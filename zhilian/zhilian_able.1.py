...
class ZLSpider(object):
    def __init__(self):
        self.url = 'https://fe-api.zhaopin.com/c/i/sou?pageSize=90&cityId=530&workExperience=-1&education=-1&companyType=-1&employmentType=-1&jobWelfareTag=-1&kw=java&kt=3&_v=0.95260331&x-zp-page-request-id=595f636d6e5c4ab9853f3217b841582d-1550714982546-78357&start={}'
    ...
    def parse_json(self, json_str):
        ...
        if results:
            for zhiwei_dict in results:
                # 公司名称
                companyName = zhiwei_dict['company']
                # 工作年限
                workExperience = zhiwei_dict['workingExp']
                # 学历
                eduLevel = zhiwei_dict['eduLevel']
                # 福利待遇
                ...
                db.zhilian_java_BJ.insert({'companyName':companyName['name'],'eduLevel':eduLevel['name'],' jobName': jobName,'workExperience':workExperience['name'],'jobURL':detail_url,'welfare':welfare,'salary':salary,'responsibility':content})
                print(companyName['name'],workExperience['name'],detail_url,welfare, salary, jobName, content,eduLevel['name'])
                # print(eduLevel)
            # 继续请求后续的数据
            self.offset += 90
            j = self.get_json_str()
    ...
    def get_detail(self, detail_url):
        ...
        content = re.search(re.compile(r'<div class="describtion".*?>(.*?)</div>', re.S), html).group(1)
